...
@TeleOp(name = "ColorDistance_Sample (Blocks to Java)")
public class ColorDistance_Sample extends LinearOpMode {

  private ColorSensor csBack_REV_ColorRangeSensor;
  ...
  public void runOpMode() {
    ...
    csBack_REV_ColorRangeSensor = hardwareMap.get(ColorSensor.class, "csBack");
    ...
    if (opModeIsActive()) {
      ...
      while (opModeIsActive()) {
        ...
        telemetry.addData("Dist to tgt (cm)", ((DistanceSensor) csBack_REV_ColorRangeSensor).getDistance(DistanceUnit.CM));
        // Display reflected light.
        telemetry.addData("Light detected", ((OpticalDistanceSensor) csBack_REV_ColorRangeSensor).getLightDetected());
        // Adjust the gain.
        if (gamepad1.a) {
          gain += 0.005;
        ...
        ((NormalizedColorSensor) csBack_REV_ColorRangeSensor).setGain(gain);
        telemetry.addData("Gain", ((NormalizedColorSensor) csBack_REV_ColorRangeSensor).getGain());
        // Read color from the sensor.
        normalizedColors = ((NormalizedColorSensor) csBack_REV_ColorRangeSensor).getNormalizedColors();
