...
@Autonomous(
  name = "F2 to Backstage",
  group = "Red",
  preselectTeleOp = "Scooter Drivetrain 2023"
)
public class F2toBackStage extends LinearOpMode {
  ...
  public void runOpMode() {
    ...
    waitForStart();
    if (opModeIsActive()) {
      DriveRobotEncoder(1, 0.5, 4, (nSquareSize + 3)); // Strafe left 27 inches from wall
      resetHeading();

      driveStraight(DRIVE_SPEED, (nSquareSize * 2.5), 0); // Drive straight for 26.5 inches

      DriveRobotEncoder(1, 0.5, 5, (nSquareSize - 2.5)); // Strafe right 24 inches
      resetHeading();

      driveStraight(DRIVE_SPEED, (nSquareSize - 6), 0); // Drive straight to get to the back stage
